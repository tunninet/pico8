...
networking_baremetal_pica8_group = cfg.OptGroup(
    name='networking_baremetal_pica8',
    title='ML2 networking-baremetal options')
CONF.register_group(networking_baremetal_pica8_group)
CONF.register_opts(_opts, group=networking_baremetal_pica8_group)
for device in CONF.networking_baremetal_pica8.enabled_devices:
    group = cfg.OptGroup(
        name=device,
...
def list_opts():
    return [('networking_baremetal_pica8', _opts)]
...
def get_devices():
    ...
    """
    devices = dict()
    for dev in CONF.networking_baremetal_pica8.enabled_devices:
